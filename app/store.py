...
class Store:
    ...
    def list_chats(self) -> list[dict]:
        return sorted(self.chats.all(), key=lambda x: x.get("last_activity_at", ""), reverse=True)

    def get_chat(self, chat_id: str) -> dict | None:
        return self.chats.get(Query().id == chat_id)

    def create_chat(self, agent_id: str, session_id: str, status: str = "starting") -> dict:
        timestamp = now_iso()
        item = {"id": str(uuid4()), "session_id": session_id, "agent_id": agent_id,
                "title": "New conversation", "status": status, "created_at": timestamp,
                "updated_at": timestamp, "last_activity_at": timestamp}
